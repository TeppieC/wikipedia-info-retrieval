...
For q8:
	...
	**
	our database containg the test RDF is called test.db and it has already has the table called statement storing RDF data in it.
	If you would like to try store the RDF in a new database please using:
		sqlite3 <name_of_db>.db --> create a new database
		.quit --> quit sqlite
		python3 q8.py <name_of_db>.db <name_of_rdf>.txt
	**
...
FOR q9:
	...
	**
	As mentioned in q8, we have the test.db to store our test RDF, you can run directly with our query test files.
	Or if you have a new database in q8, please refer to the new one.
	**
